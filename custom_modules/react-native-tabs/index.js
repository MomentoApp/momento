...
class Tabs extends Component {
    ...
    render(){
        const self = this;
        let selected = this.props.tabNumber;
        if (!selected){
                ...
                {React.Children.map(this.props.children.filter(c=>c),(el)=>
                    <TouchableOpacity key={el.props.name+"touch"}
                       ...
                       onLongPress={()=>self.props.locked && self.onSelect(el)}>
                         {selected == (el.key || el.props.name) ? React.cloneElement(el, {selected: true, style: [el.props.style, this.props.selectedStyle, el.props.selectedStyle]}) : el}
                    </TouchableOpacity>
                )}
            </View>
        );
